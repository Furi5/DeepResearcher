...
class AgentsUtils:
    ...
    async def print_stream_events(
        ...
        Args:
            result: Stream events from Runner.run_streamed()
            callback: Optional callback function to receive text deltas
        """
                ...
                if event.data.type == "response.output_item.added":
                    match event.data.item.type:
                        ...
                        case "message":
                            pass
                        case "function_call":
                            output = f"<toolcall name={event.data.item.name}>{event.data.item.arguments}"
                            PrintUtils.print_bot(output, end="")
                            if callback:
                                callback(output)
                        case _:
                            output = f"<{event.data.item.type}>"
                            PrintUtils.print_bot(output, end="")
                            if callback:
                                callback(output)
                    ...
                    match event.data.item.type:
                        ...
                        case _:
                            ...
                            # if callback:
                            #     callback(f"</{event.data.item.type}>")
                    ...
                    match event.data.part.type:
                        ...
                        case "output_text":
                            pass
                        case "refusal":
                            output = f"<refusal>{event.data.part.refusal}"
                            PrintUtils.print_bot(output, end="")
                            if callback:
                                callback(output)
                        case _:
